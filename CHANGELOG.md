...
### 新增
- **卦象导出日柱列补齐（次新股 / ETF）**：
  - Excel 日柱表（`日柱(1).xlsx`）是静态快照，只覆盖约 5395 只股票、无 ETF；次新股与全部 ETF 的日柱列此前为空
  - `ensure_rizhu_coverage`：按上市日期（Tushare `stock_basic` / `fund_basic` 的 list_date）推算 60 甲子日干支补齐缺失代码，锚点 1899-12-22=甲子，口径=上市日当天干支（已与 Excel 表已知值对拍 100% 一致）
  - 上市日期缓存落盘 `storage/astock/rizhu_list_dates.json`，缓存命中不调网络；Tushare 不可用时静默降级，不中断导出
  - Excel 表值永远优先（补齐结果在前、Excel 表覆盖）；导出 meta 新增 `rizhu_note` 说明口径
  - 新增测试：60 甲子对拍、缓存命中/落盘、Tushare 失败降级、导出补齐链路
